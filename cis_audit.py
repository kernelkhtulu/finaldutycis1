...
# Copyright (C) 2024 Andy Dustin <andy.dustin@gmail.com>
# This work is licensed under a Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License.
# https://creativecommons.org/licenses/by-nc-sa/4.0/

# This unofficial tool checks for your system against published CIS Hardening Benchmarks and offers an indication of your
# system's preparedness for compliance to the official standard.
...
def _is_test_included(test_id, test_level) -> bool:
    ...
    log.debug(f'Checking whether to run test {test_id}')

    is_test_included = True

    ## Check if the level is one we're going to run
    if CONFIG.level != 0:
        if test_level != CONFIG.level:
            log.debug(f'Excluding level {test_level} test {test_id}')
    ...
    if CONFIG.includes:
        ...
        if test_id in CONFIG.includes:
            log.debug(f'Test {test_id} was explicitly included')
            is_test_included = True

        elif is_parent_test:
            log.debug(f'Test {test_id} is the parent of an included test')
            is_test_included = True

        elif is_child_test:
            log.debug(f'Test {test_id} is the child of an included test')
            is_test_included = True

        elif CONFIG.level == 0:
            log.debug(f'Excluding test {test_id} (Not found in the include list)')
            is_test_included = False
    ...
    if CONFIG.excludes:
        ...
        if test_id in CONFIG.excludes:
            log.debug(f'Test {test_id} was explicitly excluded')
            is_test_included = False

        elif is_parent_excluded:
            log.debug(f'Test {test_id} is the child of an excluded test')
            is_test_included = False

    if is_test_included:
        log.debug(f'Including test {test_id}')
    else:
...
def audit_at_is_restricted_to_authorized_users() -> int:
    ...
    if audit_file_permissions(file="/etc/at.allow", expected_user="root", expected_group="root", expected_mode="0600") != 0:
        state += 2
...
def audit_auditing_for_processes_prior_to_start_is_enabled() -> int:
    ...
    state = 0
    efidirfile = _shellexec(R"find /boot/efi/EFI/ -type f -name 'grub.cfg' | grep -v BOOT").stdout[0]
    grubdirfile = _shellexec(R"find /boot -mindepth 1 -maxdepth 2 -type f -name 'grub.cfg'").stdout[0]
...
def audit_auth_for_single_user_mode() -> int:
    state = 0
    success_strings = [
...
def audit_bootloader_password_is_set() -> int:
    state = 0
...
def audit_chrony_is_configured() -> int:
    state = 0
...
def audit_cron_is_restricted_to_authorized_users() -> int:
    ...
    if not os.path.exists('/etc/cron.allow'):
        ...
        if audit_file_permissions(file="/etc/cron.allow", expected_user="root", expected_group="root", expected_mode="0600") != 0:
            state += 4
...
def audit_etc_passwd_gids_exist_in_etc_group() -> int:
    ...
    for gid in gids_from_etc_passwd:
        if gid not in gids_from_etc_group:
            log.warning(f'GID {gid} exists in /etc/passwd but not in /etc/group')
            state = 1
...
def audit_file_permissions(file: str, expected_mode: str, expected_user: Optional[str] = None, expected_group: Optional[str] = None) -> int:
    ...
    try:
        ...
        log.debug(f'Error trying to stat file {file}: "{e}"')
        return -1
    ...
    if expected_user is not None:
        ...
        if file_user != expected_user:
            state += 1
            log.debug(f'Test failure: file_user "{file_user}" for {file} did not match expected_user "{expected_user}"')

    if expected_group is not None:
        ## Set fail state if group does not match expecation
        if file_group != expected_group:
            state += 2
            log.debug(f'Test failure: file_group "{file_group}" for {file} did not match expected_group "{expected_group}"')
    ...
    for i in range(len(binary_file_mode)):
        if binary_expected_mode[i] == '0':
            if binary_file_mode[i] != '0':
                ...
                log.debug(f'Test comparison for {file}, {octal_expected_mode}>={octal_file_mode} {binary_expected_mode[i]} == {binary_file_mode[i]}. Failed at index {i}. Adding {this_failure_score} to state')
            else:
                log.debug(f'Test comparison for {file}, {octal_expected_mode}>={octal_file_mode} {binary_expected_mode[i]} == {binary_file_mode[i]}. Passed at index {i}')
...
def audit_homedirs_exist() -> int:
    ...
    for user, uid, homedir in _get_homedirs():
        if homedir != '':
            if not os.path.isdir(homedir):
                log.warning(f'The homedir {homedir} does not exist')
                state = 1
...
def audit_homedirs_ownership() -> int:
    ...
    for user, uid, homedir in _get_homedirs():
        ...
        if dir.st_uid != int(uid):
            state = 1
            log.warning(f'{user}({uid}) does not own {homedir}')
...
def audit_homedirs_permissions() -> int:
    ...
    for user, uid, homedir in _get_homedirs():
        if audit_file_permissions(homedir, '0750') != 0:
            state = 1
            log.warning(f'Homedir {homedir} is not 0750 or more restrictive')
...
def audit_no_unconfined_services() -> int:
    state = 0
    ...
    for option in options:
        for line in r.stdout:
            if option not in line:
                state += 8
                log.debug(f'Option "{option}" not in line "{line}"')
                break
        else:
            continue
        break
...
def audit_package_is_installed(package: str) -> int:
    cmd = f'rpm -q {package}'
    r = _shellexec(cmd)
...
def audit_package_not_installed_or_service_is_masked(package: str, service: str) -> int:
    ...
    package_installed = bool(not audit_package_is_installed(package))
    log.debug(f'package_installed = {package_installed}')

    if package_installed:
        service_masked = bool(not audit_service_is_masked(service))
        log.debug(f'service_masked = {service_masked}')
...
def audit_permissions_on_private_host_key_files() -> int:
    ...
    for counter, file in enumerate(files):
        result = audit_file_permissions(file=file, expected_user="root", expected_group="root", expected_mode="0600")
...
def audit_permissions_on_public_host_key_files() -> int:
    ...
    ## Check file permissions using audit_file_permissions()
    for counter, file in enumerate(files):
        result = audit_file_permissions(file=file + '.pub', expected_user="root", expected_group="root", expected_mode="0644")
...
def audit_selinux_mode_is_enforcing() -> int:
    state = 0
...
def audit_selinux_mode_not_disabled() -> int:
    state = 0
...
def audit_selinux_not_disabled_in_bootloader() -> int:
    state = 0
    file_paths = []
...
def audit_shadow_group_is_empty() -> int:
    ...
    gid = _shellexec("awk -F: '/^shadow:/ {print $3}' /etc/group").stdout[0]
...
def audit_sysctl_flags_are_set(flags: "list[str]", value: int) -> int:
    state = 0
...
def audit_system_accounts_are_secured() -> int:
    ...
    passwd_file = _shellexec('cat /etc/passwd').stdout
    ...
    log.debug(f'uid_min = {uid_min}')
    log.debug(f'{passwd_file}')
...
def audit_xdmcp_not_enabled() -> int:
    state = 0
...
def outputter(format: str, results: list, host_os: str, benchmark_version: str, stats: dict) -> None:
    if format in ['csv', 'psv', 'tsv']:
        ...
        output_csv(results=results, separator=separator, host_os=host_os, benchmark_version=benchmark_version)

    elif format == 'json':
        output_json(results=results, host_os=host_os, benchmark_version=benchmark_version, stats=stats)

    elif format == 'text':
        output_text(results=results, host_os=host_os, benchmark_version=benchmark_version, stats=stats)


def output_csv(results: list, separator: str, host_os, benchmark_version):
...
def output_json(results, host_os, benchmark_version, stats):
    output = {}
...
def output_text(results, host_os, benchmark_version, stats):
...
def run_tests(tests: "list[dict]"):
    ...
    for test in tests:
        ...
        if "type" in test:
            ...
            log.debug(f'Test {test_id} does not explicitly define a type, so assuming it is a test')
            test_type = 'test'
        ...
        if _is_test_included(test_id, test_level):
            if test_type == "header":
                ...
                try:
                    if kwargs and test_function:
                        log.debug(f'Requesting test {test_id}, {test_function.__name__} with kwargs: {kwargs}')
                        state = test_function(**kwargs)
                    elif test_function:
                        log.debug(f'Requesting test {test_id}, {test_function.__name__}')
                        state = test_function()

                except Exception as e:
                    log.warning(f'Test {test_id} encountered an error: "{e}"')
                    state = -1

                end_time = _get_utcnow()
                test_duration = f'{int((end_time.microsecond - start_time.microsecond) / 1000)}ms'

                if state == 0:
                    log.debug(f'Test {test_id} passed')
                    result = "Pass"
                elif state == -1:
                    result = "Error"
                elif state == -2:
                    ...
                    log.debug(f'Test {test_id} failed with state {state}')
                    result = "Fail"
...
benchmarks = {
    'CentOS 7': {
        '3.1.2': [
            ...
            {'_id': "3.3.1", 'description': "Ensure source routed packets are not accepted", 'function': audit_sysctl_flags_are_set, 'kwargs': {'flags': ["net.ipv4.conf.all.accept_source_route", "net.ipv4.conf.default.accept_source_route", "net.ipv6.conf.all.accept_source_route", "net.ipv6.conf.default.accept_source_route"], 'value': 0}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.3.2", 'description': "Ensure ICMP redirects are not accepted", 'function': audit_sysctl_flags_are_set, 'kwargs': {'flags': ["net.ipv4.conf.all.accept_redirects", "net.ipv4.conf.default.accept_redirects", "net.ipv6.conf.all.accept_redirects", "net.ipv6.conf.default.accept_redirects"], 'value': 0}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.3.3", 'description': "Ensure secure ICMP redirects are not accepted", 'function': audit_sysctl_flags_are_set, 'kwargs': {'flags': ["net.ipv4.conf.all.secure_redirects", "net.ipv4.conf.default.secure_redirects"], 'value': 0}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.3.4", 'description': "Ensure suspicious packets are logged", 'function': audit_sysctl_flags_are_set, 'kwargs': {'flags': ["net.ipv4.conf.all.log_martians", "net.ipv4.conf.default.log_martians"], 'value': 1}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.3.5", 'description': "Ensure broadcast ICMP requests are ignored", 'function': audit_sysctl_flags_are_set, 'kwargs': {'flags': ["net.ipv4.icmp_echo_ignore_broadcasts"], 'value': 1}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.3.6", 'description': "Ensure bogus ICMP responses are ignored", 'function': audit_sysctl_flags_are_set, 'kwargs': {'flags': ["net.ipv4.icmp_ignore_bogus_error_responses"], 'value': 1}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "3.5.1.2", 'description': "Ensure iptables-services not installed with firewalld", 'function': audit_package_not_installed, 'kwargs': {'package': "iptables-services"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.1.3", 'description': "Ensure nftables not installed with firewalld", 'function': audit_package_not_installed, 'kwargs': {'package': "nftables"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.1.4", 'description': "Ensure firewalld service enabled and running", 'function': audit_service_is_enabled_and_is_active, 'kwargs': {'service': "firewalld"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.1.5", 'description': "Ensure firewalld default zone is set", 'function': audit_firewalld_default_zone_is_set, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "3.5.2.1", 'description': "Ensure nftables is installed", 'function': audit_package_is_installed, 'kwargs': {'package': "nftables"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.2", 'description': "Ensure firewalld is not installed", 'function': audit_package_not_installed, 'kwargs': {'package': "firewalld"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.3", 'description': "Ensure iptables-services not installed with nftables", 'function': audit_package_not_installed, 'kwargs': {'package': "iptables-services"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.4", 'description': "Ensure iptables are flushed with nftables", 'function': audit_iptables_is_flushed, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.5", 'description': "Ensure an nftables table exists", 'function': audit_nftables_table_exists, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.6", 'description': "Ensure nftables base chains exist", 'function': audit_nftables_base_chains_exist, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.7", 'description': "Ensure nftables loopback traffic is configured", 'function': audit_nftables_loopback_is_configured, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.8", 'description': "Ensure nftables outbound and established connections are configured", 'function': audit_nftables_outbound_and_established_connections, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.9", 'description': "Ensure nftables default deny firewall policy exists", 'function': audit_nftables_default_deny_policy, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "3.5.2.10", 'description': "Ensure nftables service is enabled", 'function': audit_service_is_enabled, 'kwargs': {'service': "nftables"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "3.5.3.1.1", 'description': "Ensure iptables-services is installed", 'function': audit_package_is_installed, 'kwargs': {'package': "iptables-services"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "4.1.2.1", 'description': "Ensure audit log storage size is configured", 'function': audit_audit_log_size_is_configured, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.2.2", 'description': "Ensure audit logs are not automatically deleted", 'function': audit_audit_logs_not_automatically_deleted, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.2.3", 'description': "Ensure system is disabled when audit logs are full", 'function': audit_system_is_disabled_when_audit_logs_are_full, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.2.4", 'description': "Ensure audit_backlog_limit is sufficient", 'function': None, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.3", 'description': "Ensure events that modify date and time information are collected", 'function': audit_events_that_modify_datetime_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.4", 'description': "Ensure events that modify user/group information are collected", 'function': audit_events_that_modify_usergroup_info_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.5", 'description': "Ensure events that modify the system's network environment are collected", 'function': audit_events_that_modify_network_environment_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.6", 'description': "Ensure events tat modify the system's Mandatory Access Controls are collected", 'function': audit_events_that_modify_mandatory_access_controls_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.7", 'description': "Ensure login and logout events are collected", 'function': audit_events_for_login_and_logout_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.8", 'description': "Ensure session initiation information is collected", 'function': audit_events_for_discretionary_access_control_changes_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.9", 'description': "Ensure discretionary access control permissions modification events are collected", 'function': audit_events_for_discretionary_access_control_changes_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.10", 'description': "Ensure unsuccessful unauthorized file access attempts are collected", 'function': audit_events_for_unsuccessful_file_access_attempts_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.11", 'description': "Ensure use of privileged commands is collected", 'function': None, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.12", 'description': "Ensure successful file system mounts are collected", 'function': audit_events_for_successful_file_system_mounts_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.13", 'description': "Ensure file deletion events by users are collected", 'function': audit_events_for_file_deletion_by_users_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.14", 'description': "Ensure changes to system administration scope (sudoers) is collected", 'function': audit_events_for_changes_to_sysadmin_scope_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.15", 'description': "Ensure system administrator command executions (sudo)are collected", 'function': audit_events_for_system_administrator_commands_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.16", 'description': "Ensure kernel module loading and unloading is collected", 'function': audit_events_for_kernel_module_loading_and_unloading_are_collected, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "4.1.17", 'description': "Ensure the audit configuration is immutable", 'function': audit_audit_config_is_immutable, 'levels': {'server': 2, 'workstation': 2}},
            ...
            {'_id': "4.2.2.1", 'description': "Ensure journald is configured to send logs to rsyslog", 'function': audit_journald_configured_to_send_logs_to_rsyslog, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "4.2.2.2", 'description': "Ensure journald is configured to compress large log files", 'function': audit_journald_configured_to_compress_large_logs, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "4.2.2.3", 'description': "Ensure journald is configured to write logfiles to persistent disk", 'function': audit_journald_configured_to_write_logfiles_to_disk, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "4.2.3", 'description': "Ensure permissions on all logfiles are configured", 'function': audit_permissions_on_log_files, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.1.5", 'description': "Ensure permissions on /etc/cron.weekly are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/cron.weekly", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0700"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.1.6", 'description': "Ensure permissions on /etc/cron.monthly are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/cron.monthly", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0700"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.1.7", 'description': "Ensure permissions on /etc/cron.d are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/cron.d", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0700"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.3.1", 'description': "Ensure permissions on /etc/ssh/sshd_config are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/ssh/sshd_config", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0600"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.2", 'description': "Ensure permissions on SSH private host key files are configured", 'function': audit_permissions_on_private_host_key_files, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.3", 'description': "Ensure permissions on SSH public host key files are configures", 'function': audit_permissions_on_public_host_key_files, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.3.6", 'description': "Ensure SSH X11 forwarding is disabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "x11forwarding", 'expected_value': "no"}, 'levels': {'server': 2, 'workstation': 1}},
            {'_id': "5.3.7", 'description': "Ensure SSH MaxAuthTries is set to 4 or less", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "maxauthtries", 'expected_value': "4", 'comparison': "le"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.8", 'description': "Ensure SSH IgnoreRhosts is enabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "ignorerhosts", 'expected_value': "yes"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.9", 'description': "Ensure SSH HostbasedAuthentication is disabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "hostbasedauthentication", 'expected_value': "no"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.10", 'description': "Ensure SSH root login is disabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "permitrootlogin", 'expected_value': "no"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.11", 'description': "Ensure SSH PermitEmptyPasswords is disabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "permitemptypasswords", 'expected_value': "no"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.12", 'description': "Ensure SSH PermitUserEnvironment is disabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "permituserenvironment", 'expected_value': "no"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.3.12.1", 'description': "Ensure SSH ClientAliveInterval is 900 or less", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "clientaliveinterval", 'expected_value': "900", 'comparison': "le"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.12.2", 'description': "Ensure SSH ClientAliveCountMax is 0", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "clientalivecountmax", 'expected_value': "0"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.17", 'description': "Ensure SSH LoginGraceTime is set to one minute or less", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "logingracetime", 'expected_value': "60", 'comparison': "le"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.18", 'description': "Ensure SSH warning banner is configured", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "banner", 'expected_value': "/etc/issue.net"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.19", 'description': "Ensure SSH PAM is enabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "usepam", 'expected_value': "yes"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.20", 'description': "Ensure SSH AllowTcpForwarding is disabled", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "allowtcpforwarding", 'expected_value': "no"}, 'levels': {'server': 2, 'workstation': 2}},
            {'_id': "5.3.21", 'description': "Ensure SSH MaxStartups is configured", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "maxstartups", 'expected_value': "10:30:60"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.3.22", 'description': "Ensure SSH MaxSessions is limited", 'function': audit_sshd_config_option, 'kwargs': {'parameter': "maxsessions", 'expected_value': "10", 'comparison': "le"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.4.3", 'description': "Ensure password hashing algorithm is SHA512", 'function': audit_password_hashing_algorithm, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.4.4", 'description': "Ensure password reuse is limited", 'function': audit_password_reuse_is_limited, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.5.1.1", 'description': "Ensure password expiration is 365 days or less", 'function': audit_password_expiration_max_days_is_configured, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.5.1.2", 'description': "Ensure minimum days between password changes is configured", 'function': audit_password_change_minimum_delay, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.5.1.3", 'description': "Ensure password expiration warning days is 7 or more", 'function': audit_password_expiration_warning_is_configured, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.5.1.4", 'description': "Ensure inactive password lock is 30 days or less", 'function': audit_password_inactive_lock_is_configured, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.5.2", 'description': "Ensure system accounts are secured", 'function': audit_system_accounts_are_secured, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "5.5.3", 'description': "Ensure default group for the root account is GID 0", 'function': audit_default_group_for_root, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "5.7", 'description': "Ensure access to the su command is restricted", 'function': audit_access_to_su_command_is_restricted, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "6.1.3", 'description': "Ensure permissions on /etc/passwd- are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/passwd-", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0644"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.1.4", 'description': "Ensure permissions on /etc/shadow are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/shadow", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0000"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "6.1.6", 'description': "Ensure permissions on /etc/gshadow- are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/gshadow-", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0000"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "6.1.8", 'description': "Ensure permissions on /etc/group are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/group", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0644"}, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.1.9", 'description': "Ensure permissions on /etc/group- are configured", 'function': audit_file_permissions, 'kwargs': {'file': "/etc/group-", 'expected_user': "root", 'expected_group': "root", 'expected_mode': "0644"}, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "6.2.4", 'description': "Ensure shadow group is empty", 'function': audit_shadow_group_is_empty, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.2.5", 'description': "Ensure no duplicate user names exist", 'function': audit_duplicate_user_names, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.2.6", 'description': "Ensure no duplicate group names exist", 'function': audit_duplicate_group_names, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.2.7", 'description': "Ensure no duplicate UIDs exist", 'function': audit_duplicate_uids, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.2.8", 'description': "Ensure no duplicate GIDs exist", 'function': audit_duplicate_gids, 'levels': {'server': 1, 'workstation': 1}},
            ...
            {'_id': "6.2.11", 'description': "Ensure all users' home directories exist", 'function': audit_homedirs_exist, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.2.12", 'description': "Ensure users own their home directories", 'function': audit_homedirs_ownership, 'levels': {'server': 1, 'workstation': 1}},
            {'_id': "6.2.13", 'description': "Ensure users' home directory permissions are 750 or more restrictive", 'function': audit_homedirs_permissions, 'levels': {'server': 1, 'workstation': 1}},
