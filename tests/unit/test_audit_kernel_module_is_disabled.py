...
from cis_audit import audit_kernel_module_is_disabled


def mock_module_disabled(cmd):
    if 'modprobe' in cmd:
...
@patch("cis_audit._shellexec", mock_module_disabled)
def test_audit_kernel_module_is_disabled_pass_disabled():
    state = audit_kernel_module_is_disabled(module='pytest')
    assert state == 0


@patch("cis_audit._shellexec", mock_filesystem_not_found)
def test_audit_kernel_module_is_disabled_pass_not_found():
    state = audit_kernel_module_is_disabled(module='pytest')
    assert state == 0


@patch("cis_audit._shellexec", mock_module_enabled)
